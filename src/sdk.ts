...
export async function setupWithContract(
  providerOpt: Web3Provider,
  pnsContract: IPNS,
  registrarContract: IController,
  resolverContract: IResolver
) {
  await setProvider(providerOpt);
...
export async function totalRegisterPrice(
  name: LabelString,
  duration: number
): Promise<BigNumber> {
...
export async function registerWithConfig(
  ...
  const price = await totalRegisterPrice(label, duration);
  return controller.nameRegisterWithConfig(
    label,
    account,
    duration,
    resolver,
...
async function fetchResolver(
  ...
  if (!resv) {
    ...
    return currResolver !== emptyAddress
      ? resolver.attach(currResolver)
      : resolver;
  } else {
    return resv;
  }
...
export async function setNftName(
  nftAddr: HexAddress,
  nftTokenId: string,
  nameTokenId: TokenId
) {
...
export async function setKeysByHash(
  name: DomainString,
  keys: string[],
  values: string[],
  resv?: IResolver
...
export async function getKeysByHash(
  name: DomainString,
  key: string[],
  resv?: IResolver
) {
...
export function getLabelhash(rawlabel: string): HexAddress {
  ...
  return rawlabel.startsWith("[") &&
    rawlabel.endsWith("]") &&
    rawlabel.length === 66
    ? "0x" + rawlabel.slice(1, -1)
    : "0x" + keccak_256(rawlabel);
}
...
export async function transferName(name: DomainString, newOwner: HexAddress) {
  ...
  return pns["safeTransferFrom(address,address,uint256)"](
    oldOwner,
    newOwner,
    namehash
  );
